...
class Tcp
{
    ...
    protected function send($ip, $port, $cmd, $timeOut = 2, $toJson = false, $close = true)
    {
        ...
        // 执行结束，执行读取数据钩子
        TcpHook::listen('send_after', ['tag' => 'send_after', 'response' => $data, 'cmd' => $cmd, 'ip' => $ip, 'port' => $port]);
        ...
        // 执行返回结果集钩子
        TcpHook::listen('result_return', ['tag' => 'result_return', 'response' => $data, 'result' => $result]);
        return $result;
    }
}
