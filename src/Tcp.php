...
class Tcp
{
    ...
    protected static function send($ip, $port, $cmd, $timeOut = 2, $toJson = false, $close = true)
    {
        ...
        if (!$socket) {
            // 执行创建Socket失败钩子
            TcpHook::listen('create_faild', ['error' => socket_strerror(socket_last_error()), 'cmd' => $cmd, 'ip' => $ip, 'port' => $port]);
        ...
        if (socket_connect($socket, $ip, $port) == false) {
            // 执行链接Socket失败钩子
            TcpHook::listen('connect_faild', ['error' => socket_strerror(socket_last_error()), 'cmd' => $cmd, 'ip' => $ip, 'port' => $port]);
        ...
        if ($sent != $send_len) {
            // 执行发送CMD指令失败钩子
            TcpHook::listen('sned_faild', ['error' => socket_strerror(socket_last_error()), 'cmd' => $cmd, 'ip' => $ip, 'port' => $port]);
            return self::errorQuit('发送CMD指令失败');;
        }
        // 读取返回数据
        $data = socket_read($socket, 1024);
        // 执行结束，执行读取数据钩子
        TcpHook::listen('send_after', ['response' => $data, 'cmd' => $cmd, 'ip' => $ip, 'port' => $port]);
